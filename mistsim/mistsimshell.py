...
def precomputeclusters(corecalls, outrand, subset, replicates, cores):
    '''
    creates randomly subsetted genes and gives out a csv file containing cutree of them.
    Also makes a reference file at cutree h= column numbers
    '''
    if not os.access(outrand, os.F_OK):
        os.mkdir(outrand)
    prearg=('Rscript', 'pre_compute_clusters.R',
            '-i', corecalls,
            '-n', subset,
            '-r', replicates,
            '-c', cores,
            '-o', outrand)
    subprocess.call(prearg)

def comparesubtoref(outrand, outfile, cores, outplot, replicates, subset):
    '''
    calculates adjusted wallaces from the reference and subsetted genes generated by precomputeclusters
    '''
    comparg=('Rscript', 'compare_gene_subsets_to_reference.R',
             '-i', outrand,
             '-o', outfile,
             '-c', cores,
             '-p', outplot,
...
def arguments():
    ...
    c_parser = subparsers.add_parser('Random')
    c_parser.add_argument('--subset', default='404', help='number of genes to subset randomly')
    c_parser.add_argument('--replicates', default='4', help='number of times to subset')

    c_parser.add_argument('--corecalls', default='/home/phac/kye/autocreate/prissy/core_calls.csv', help='core_calls.csv file that autocreate generated')
    c_parser.add_argument('--outrand', default='/home/phac/kye/misty/ranclusters', help='directory to place files in')
    c_parser.add_argument('--outfile', default='report.csv', help='name of csv file to save adjusted wallace results in')
    c_parser.add_argument('--outplot', default='viograph.png', help='name of the violin graph')

    c_parser.add_argument('--cores', default=multiprocessing.cpu_count())


    d_parser = subparsers.add_parser('Diversity')
    d_parser.add_argument('--file', default='0', help='lowest chopped.csv file, contains all genes needed')
    d_parser.add_argument('--chopout', default='chopsyms/', help='access the directory of symlinked gene files')
    d_parser.add_argument('--reportdir', default='/home/phac/kye/misty/mistout/', help='directory of all genome report files generated by MIST')
    d_parser.add_argument('--pristine', default='/home/phac/genomes/campylobacter/pristine/', help='pristine genomes directory, which this is made to test')
    d_parser.add_argument('--output', default='diversity/', help='where to place the files')
    d_parser.add_argument('--outfile', default='diversity.json', help='name of the output json file')

    d_parser.add_argument('--start', default=0, help='where to start, in terms of worst gene to compare')
    d_parser.add_argument('--end', default=10, help='where to end the comparison of gene alleles')
...
def processSim(path, outpath, outfile, genemin, genethreshhold, genomemin, genomethreshhold,
    ...
    '''
    pretends to remove genes that are missing from more genomes than a given threshold,returns a graph
    of how many genomes and genes remain. Then pretends to remove genes in order from worst to best,
...
def processChop(path, outpath, outfile, outcut, startpop, endpop, startchop, chopout, alleles, corecalls, cores):
    '''
...
def processRandom(corecalls, outrand, subset, replicates, outfile, outplot, cores):
    '''
    randomly subsets genes and calculates adjusted wallace values of them against a generated reference file
    '''
    print('subsetting genes and making clusters/reference file')
    precomputeclusters(corecalls, outrand, subset, replicates, cores)
    print('calculating adjusted wallaces')
    comparesubtoref(outrand, outfile, cores, outplot, replicates, subset)
...
def main():
    ...
    if args.subfunction == 'Chop':
        processChop(args.path, args.outpath, args.outfile, args.outcut, args.startpop, args.endpop,
                    args.startchop, args.chopout, args.alleles, args.corecalls, args.cores)

    if args.subfunction == 'Random':
        processRandom(args.corecalls, args.outrand, args.subset, args.replicates,
                      args.outfile, args.outplot, args.cores)
